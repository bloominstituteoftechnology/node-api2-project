...
router.post('/', (req, res) => {
  ...
  if(!req.body.title || !req.body.contents){
    ...
    database.insert({title: req.body.title, contents: req.body.contents})
      .then(data => { res.status(201).json({title: req.body.title, contents: req.body.contents, id: data.id})})
...
router.post('/:id/comments', (req, res) => {
  if (!req.body.text){
    ...
    database.findPostComments(req.params.id)
      .then(data =>{
        if(data.length > 0){
          database.insertComment({text:req.body.text, post_id: req.params.id})
            .then(data2 => {
    ...
    .then(data => {
      if(data.length > 0){
        database.remove(req.params.id)
          ...
          .catch(err => { res.status(500).json({errorMessage:"The post could not be removed."})})
      } else {
        res.status(404).json({errorMessage:"The post with the specified ID does not exit."});
    }
    })
...
router.put('/:id', (req, res) => {
  if(!req.body.title || !req.body.contents){
    ...
    database.update(req.params.id, {title:req.body.title, contents:req.body.contents})
      ...
      .catch(err => {res.status(500).json({errorMessage:"The post information count not be modified."})})
  }
})
