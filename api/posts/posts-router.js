  ...
  Post.find()
    ...
    .catch((err) => {
      res
        .status(500)
        .json({
          message: `${err.message}. The posts information could not be retrieved.`,
        });
    });
});
  ...
  Post.findById(id)
    .then((post) => {
      if (post) {
        ...
        res
          .status(404)
          .json({ message: `The post with the id of ${id} does not exist.` });
      }
    })
    .catch((err) => {
      status(500).json({
        message: `The post information could not be retrieved because ${err}`,
      });
    });
});
  ...
  Post.insert(req.body)
    .then((post) => {
      if (!title || !contents) {
        res
          .status(400)
          .json({ message: "please provide titles and contents for the post" });
      } else {
    ...
    .catch((err) => {
      res
        .status(500)
        .json({
          message: `There was an ${err} while saving the post to the database.`,
        });
    });
});
