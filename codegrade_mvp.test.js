...
const post1 = {
    title: "title 1", contents: "I wish the ring had never come to me.",
};

const post2 = {
    title: "title 2", contents: "I think we should get off the road.",
};
    ...
    describe( "[PUT] /api/posts/:id", () => {
        ...
        test( "[11] responds with the correct message & status code on validation problem", async () => {
            ...
            expect( res.status )
                .toBe( 400 );
            expect( res.body.message )
                .toMatch( /provide title and contents/i );
    ...
    describe( "[GET] /api/posts/:id/comments", () => {
        ...
        test( "[16] can get all the comments associated to the posts with given id", async () => {
            ...
            expect( res.body )
                .toHaveLength( 2 );
            expect( res.body )
                .toMatchObject( [ comments[ 0 ], comments[ 1 ] ] );
            res = await request( server )
                .get( "/api/posts/2/comments" );
            expect( res.body )
                .toHaveLength( 1 );
            expect( res.body )
                .toMatchObject( [ comments[ 2 ] ] );
        }, 750 );
    } );
} );
